...
const handleLogin = async (e) => {
  ...
  try {
    ...
    if (token) {
      ...
      // Decode Token buat dapetin Supabase User ID
      ...
      try {
        ...
        // Kalau 404 brarti user baru (Not Found pofile-nya / blm nyetting role)
        if (err.status === 404) {
          navigateTo('/onboarding')
        } else {
      ...
      errorMessage.value = 'Berhasil ke API, tapi Frontend tidak bisa menemukan Access Token. Buka inspect element lalu cek tipe response-nya di Tab Console!'
    }
  } catch (error) {
      ...
      <div class="bg-white w-full max-w-md rounded-2xl shadow-[0_20px_40px_-15px_rgba(0,0,0,0.1)] border border-gray-100 p-8 sm:p-10">
        ...
        <form @submit="handleLogin" class="space-y-5">
          ...
          <div>
            <div class="flex items-center justify-between mb-1.5">
              ...
              <a href="#" class="text-[13px] font-medium text-[#2B4DB6] hover:text-blue-800 transition">Forgot Password?</a>
            </div>
            <div class="relative">
              <input 
                id="password" 
        ...
        <div class="mt-8 text-center">
          <p class="text-[14px] text-gray-600 font-normal">
            ...
            <NuxtLink to="/auth/register" class="text-[#2B4DB6] font-medium hover:text-blue-800 transition">Sign up now</NuxtLink>
          </p>
        </div>
      </div>
    </main>
