...
useHead({ title: 'CarrePath | Provider Upgrade' })
...
const loadProviderProfile = async () => {
  ...
  try {
    ...
    providerProfile.value = asObject(payload.profile || payload)
  } catch {
...
onMounted(async () => {
  try {
    ...
    const res = await get('/payment/packages?target_role=provider')
...
<template>
  <section class="p-6 md:p-8">
    <h1 class="text-[28px] font-semibold mb-2">Provider Upgrade</h1>
    <p class="text-[14px] text-[#64748B] mb-6">Upgrade ke premium agar bootcamp Anda lebih terlihat dan direkomendasikan oleh perusahaan</p>
    ...
    <!-- Packages Grid -->
    <div v-else-if="packages.length" class="grid grid-cols-1 md:grid-cols-3 gap-4">
      <div v-for="pkg in packages" :key="pkg.id" class="bg-white border border-[#E2E8F0] rounded-[10px] p-6 flex flex-col">
        <h3 class="text-[20px] font-semibold text-[#1E293B]">{{ pkg.name || pkg.package_name }}</h3>
    ...
    <div v-else class="bg-white border border-[#E2E8F0] rounded-[10px] p-8 text-center">
      <p class="text-[16px] font-medium text-[#334155]">Paket premium belum tersedia</p>
      <p class="text-[14px] text-[#64748B] mt-1">Hubungi admin untuk informasi lebih lanjut.</p>
    </div>
    ...
    <Modal :open="openModal" title="Complete Your Purchase" @close="openModal = false">
      <div class="space-y-4">
        ...
        <div v-if="selectedPackage" class="bg-[#F8FAFC] border border-[#E2E8F0] rounded-[10px] p-4">
          ...
          <p class="text-[16px] font-semibold">{{ selectedPackage.name || selectedPackage.package_name }}</p>
