...
const fetchDetail = async () => {
  ...
  try {
    ...
    const savedItem = savedList.find(s => s.id === bootcampId)
    if (savedItem) {
      isSaved.value = true
      savedId.value = savedItem.saved_id
    }
...
const toggleSave = async () => {
  if (saveLoading.value || !bootcamp.value) return
  saveLoading.value = true
  
  try {
    if (isSaved.value) {
      await del(`/saved/bootcamp/${savedId.value}`)
      isSaved.value = false
      savedId.value = ''
    } else {
    ...
    <div v-else-if="bootcamp" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div class="lg:col-span-2 space-y-6">
        ...
        <div class="bg-white border border-[#E2E8F0] rounded-[16px] p-6 shadow-sm">
          ...
          <div class="mt-6 pt-6 border-t border-[#F1F5F9]">
            <h3 class="text-[16px] font-semibold text-[#0F172A] mb-3">Deskripsi Bootcamp</h3>
          ...
          <div class="mt-8 flex gap-3">
             <button
              class="flex-1 md:flex-none flex items-center justify-center gap-2 rounded-[10px] border transition px-6 py-3 font-semibold"
              :class="isSaved ? 'border-[#93C5FD] bg-[#EFF6FF] text-[#1D4ED8] hover:bg-[#DBEAFE]' : 'border-[#CBD5E1] bg-white text-[#475569] hover:bg-[#F8FAFC]'"
              :disabled="saveLoading"
              @click="toggleSave"
            >
              <Icon v-if="saveLoading" name="heroicons:arrow-path" class="w-5 h-5 animate-spin" />
              <Icon v-else :name="isSaved ? 'heroicons:bookmark-solid' : 'heroicons:bookmark'" class="w-5 h-5" />
      ...
      <div class="space-y-6">
        ...
        <div class="bg-white border border-[#E2E8F0] rounded-[16px] p-6 shadow-sm">
          ...
          <!-- About -->
          <div class="mb-5">
            <h3 class="text-[13px] font-bold text-[#94A3B8] uppercase tracking-wider mb-2">Tentang Provider</h3>
            <p class="text-[13px] text-[#475569] leading-[1.7]">
              {{ bootcamp.provider_profiles?.description || 'Tidak ada deskripsi provider tersedia.' }}
            </p>
          </div>
          ...
          <div class="space-y-3 pt-4 border-t border-[#F1F5F9]">
            ...
            <div class="flex items-start gap-3">
               ...
               <p class="text-[13px] text-[#475569]">Dibuat: {{ formatDate(bootcamp.created_at) }}</p>
            </div>
          </div>
        </div>
      </div>
