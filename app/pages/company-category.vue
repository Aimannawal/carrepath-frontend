...
useHead({ title: 'CarrePath | Category of Company' })
...
const submit = async () => {
  ...
  try {
    ...
    await put(`/companies/profile/${payload.sub}`, { category: categories[selected.value].label })
...
<template>
  <section class="bg-[#F8F9FA] w-full min-h-screen flex items-center justify-center font-['Outfit']">
    <div class="flex flex-col items-start text-start gap-12">
      <h1 class="text-[24px] font-medium text-[#292929]">Choose your type of Company</h1>
      <form class="w-full flex flex-col items-end gap-12" @submit.prevent="submit">
        <div class="flex flex-row flex-wrap items-center gap-x-2.5 gap-y-4 max-w-[700px]">
          <div v-for="(item, index) in categories" :key="index" @click="selected = index" class="cursor-pointer transition-all duration-200 ease-in-out px-5 py-[15px] gap-[10px] rounded-[10px] flex items-center w-fit bg-white hover:shadow-sm" :class="selected === index && 'ring-2 ring-[#2B4DB6]'">
